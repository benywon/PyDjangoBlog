...
class VDiskAPIClient(object):
    ...
    def __init__(self, account, password):
        ...
        self._appkey = '1972782632'
        self._app_secret = 'af2f67c7293affd100c311e719802f0e'
        self._expires = 25 * 60
...
def UpdateData(path):
    client = VDiskAPIClient('bingning.wang@nlpr.ia.ac.cn', 'cc2015')
    client.post.auth__get_token()
    P=client.post.dir__get_dirid_with_path(path = '/BlogBackUp')
    dir_id=P['data']['id']
    client.upload.file__upload_file(dir_id = dir_id, file=path, cover='yes')
def UpdateFiles(path):
    client = VDiskAPIClient('bingning.wang@nlpr.ia.ac.cn', 'cc2015')
    client.post.auth__get_token()
    P=client.post.dir__get_dirid_with_path(path = '/BlogBackUp/Files')
    dir_id=P['data']['id']
    client.upload.file__upload_file(dir_id = dir_id, file=path, cover='yes')
...
if __name__ == '__main__':
    ...
    P=client.post.dir__get_dirid_with_path(path = '/BlogBackUp')
    dir_id=P['data']['id']
    client.upload.file__upload_file(dir_id = dir_id, file='L:\\1.JPG', cover='yes')
