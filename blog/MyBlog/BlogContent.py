...
from blog.models import  BlogsPost, BlogComments, BlogsPostDraft
...
class ArticleDraftForm(forms.ModelForm):
    title = forms.CharField(label=u'名称：',max_length=200,required=False)
    # Description=UEditorField("描述",initial="",width=1000,height=500,max_length=10000,toolbars="full",imagePath="./FileDir/blogmaterials/uploadpic/",filePath="./FileDir/blogmaterials/uploadFile/")
    category = forms.CharField(label=u'类别：',max_length = 1500,required=False)
    abstract = forms.CharField(label=u'简要介绍：',max_length = 25000,required=False)
    class Meta:
        model=BlogsPostDraft
        fields = ('title', 'body','category','abstract')
class ArticleCommentsForm(forms.Form):
    body=UEditorField("",width=650,height=160,max_length=10000,toolbars="mini",imagePath="./FileDir/blogmaterials/uploadpic/",filePath="./FileDir/blogmaterials/uploadFile/")
...
def Article(request):
    ...
    if username == '' or username is None:#检测是否已经登录
        ...
        if request.method == "POST":
            ...
            submit=request.POST.get('submitblog')
            if submit is None or submit=='':#如果是保存草稿
                contentssave=BlogsPostDraft()
            else:
                contentssave=BlogsPost()
            if uf.is_valid():
                #获取表单信息
...
def ArticleEditDraft(request):
    username = request.COOKIES.get('username')
    if username != 'god':#检测是否已经登录
        return HttpResponseRedirect('/blog/index')
    if request.method == "GET":
        showlen=20
        lengh=len(BlogsPostDraft.objects.using('Articles').all())
        num=-1
        try:
            did=request.GET['id']
            did=int(did)
        ...
        if did>=0:
            BlogsPostDraft.objects.using('Articles').filter(id__exact=did).delete()
        if alter>=0:
            post=BlogsPostDraft.objects.using('Articles').get(id=alter)#要改变的
            #跳转到更改页面
            uf=ArticleDraftForm(instance=post)
            global preeditid
            preeditid=post.id
            return render_to_response('ArticlePost.html',{'uf':uf})
        posts=BlogsPostDraft.objects.using('Articles').all().order_by('-id')[start:end]
        t=render_to_response('EditArticleDraft.html',{'posts':posts,'number':num,'pre':num-1,'next':num+1},context_instance=RequestContext(request))
        return t
    else:
        uf = ArticleDraftForm(request.POST)
        if uf.is_valid():
            global preeditid
            #获取表单信息
            title = uf.cleaned_data['title']
            ...
            submit=request.POST.get('submitblog')
            if submit is None or submit=='':#如果是保存草稿
                BlogsPostDraft.objects.using('Articles').filter(id=preeditid).update(title=title,body=Description,abstract=abstract,category=category,lastmodified=time)
            else:#真正要发表了
                BlogsPostDraft.objects.using('Articles').filter(id__exact=preeditid).delete()
                article = BlogsPost()
                article.title=title
                article.body=Description
                article.auther=u'王炳宁'
                ...
                article.timestamp=time
                article.save(using='Articles')
        return HttpResponseRedirect('/blog/Post/Article/editdraft')